...
const useViewer2D = (iafViewer, widgetModes, updateWidgetProperties) => {
  const widgetMode = 'view2d';

  useEffect(() => {
    const widget = widgetModes.find(widget => widget.evmId === EvmUtils.EVMMode.View2d);
    if (!widget) return;

    const shouldRender = _.size(iafViewer.props.fileSet2d) > 0
                              && iafViewer.props.enable2DViewer 
                              && iafViewer.props[widgetMode].enable
                              && iafViewer.state.visible;
  ...
  }, [iafViewer.props.fileSet2d, iafViewer.props.enable2DViewer, iafViewer.props[widgetMode].enable, iafViewer.state.visible, widgetModes, updateWidgetProperties]);
};
